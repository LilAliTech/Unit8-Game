...
function preload() {
  ...
  enmyCraft2 = loadImage("enmy2.png");
  enmyCraft = loadImage("enemy1.png");
}
         
function setup() {  
  canvas = createCanvas(1200,900);
  space = createSprite(250,350,30,30);
  ...
  enmyGroup = new Group;
  //endline = createSprite(250,700,1500,5);
...
function draw() {
  ...
  if(gameState === play) {
    ...
    if(keyDown("right") && spaceShip.x < 1090) {
      spaceShip.x = spaceShip.x + 15;
      p1.x = p1.x + 15;
      p2.x = p2.x + 15;
    }

    if(keyDown("left") && spaceShip.x > 140) {
      spaceShip.x = spaceShip.x - 15;
      p1.x = p1.x - 15;
      p2.x = p2.x - 15;
    }
  ...
  //  }
    ...
    text("The enemies and asteroids destroyed the planet",canvas.width/2-400,canvas.height/2+100);
  ...
  if(gameState === instruction) {
    ...
    text(" Some asteroids and enemy crafts are coming towords Earth.",canvas.width/2-300, canvas.height/2 - 210);
  ...
  if(frameCount % 110 === 0) {
    ...
    switch(rand) {
      case 1: asteroid.addImage(asteroid1);
              ...
              break;
      case 2: asteroid.addImage(enmyCraft);
              asteroid.setCollider("circle",50,0,150);
              break;
      case 3: asteroid.addImage(enmyCraft2);
...
//function enemy(){
//  var enemys = createSprite(Math.round(random(30,1300))-20);
//  enemys.velocityY = (7 +score/10);
//  enemys.lifetime = 200
//  enemys.scale = random(0.4,0.5)
//
//  var rand =
//}